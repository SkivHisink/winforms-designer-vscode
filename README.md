...
[![Version 1.9](https://img.shields.io/badge/version-1.9-brightgreen.svg)](#-support-matrix)
...
- **.NET Framework & DevExpress support** — `net4x` forms render on a bundled **.NET Framework 4.8** engine that interprets your **live source** (the Visual Studio model) onto the compiled controls (so DevExpress `XtraUserControl` & co. look pixel-accurate); the extension auto-routes each form to the right engine, and the property grid, drag/resize/align, add/remove, z-order, cut/paste, tab-page add/rename/delete/reorder, dropping the project's own vendor controls from the toolbox, and the collection editors apply live on the interpreted picture. A construct the interpreter can't yet reproduce falls back to a disclosed compiled render of the last build. Like Visual Studio, the interpreted path never constructs the class you are editing — so your form's constructor, field initializers and `Load` do not run when you open the designer — and it handles what real hand-written designer files contain: unqualified type names resolved through the file's own `using` scope, non-public constructors, and vendor collections that aren't `IList`.
- **Visual Studio–style workflow** — opening `Form.cs` opens the designer; *View Code* switches back to text.
- **Explorer project items** — right-click a local project file/folder and choose **Add → Windows Form, User Control,
  Component, or Class** ([#4](https://github.com/SkivHisink/winforms-designer-vscode/issues/4)). Forms and user controls
  are created from the Visual Studio item template itself — the same `.cs` / `.Designer.cs` pair, generated-code
  region, using block (honoring implicit usings and `.editorconfig` `csharp_using_directive_placement`), and `.resx`
  policy — in one undoable operation, and open in the
  designer; supported SDK/classic projects keep the correct namespace and item inclusion, while ambiguous or dynamic
  project shapes and file collisions are refused before any file is written.
...
- **Toolbox** - auto-populated from `System.Windows.Forms` (~39 controls in VS categories, with their native icons) plus controls lazily discovered from the open form's owning project and explicit project-reference outputs (including concrete custom output roots), configured probe directories, browsed libraries, and registered .NET assemblies. Discovery is budgeted, cancellable, metadata-only, cached by file identity, and incrementally refreshed after rebuilds. **Choose Toolbox Items** can hide/restore discovered rows, remembers workspace-specific chosen items and custom tabs across reloads, migrates existing pre-1.8 curation once, and uses the exact source assembly when adding a control or project reference. COM and WPF toolbox pages are explicitly unsupported and inert.
...
- **Direct manipulation** - select, move, resize (8 handles), keyboard nudge (arrow keys), multi-select (Ctrl/Shift + rubber-band), group move/delete, reparent, z-order, copy/paste, **duplicate** (`Ctrl+D`), **lock controls**, align + distribute + make-same-size, tab-order editor, snaplines, on-canvas **smart-tags**, and a VS-style right-click menu. Hold **Alt** during one move/resize to suppress snapping and see exact live bounds; the modifier is configurable and never bypasses geometry authorization. Modern final free-control bounds are corrected by the real WinForms graph before the source transaction is accepted. Cross-form paste validates every typed binding/extender dependency first and names any unavailable dependency without changing source.
- **Events** — describe, wire / unwire / rewire handlers, generate a handler stub, navigate to its body, or
  double-click a control to create/open its real metadata-defined default event
  ([#5](https://github.com/SkivHisink/winforms-designer-vscode/issues/5)).
...
| `winformsDesigner.net48.isolateRenderWindows` | `true` | Run the **net48** preview engine on a private desktop that is never displayed, so a form being designed cannot put windows on your screen. The compiled preview realizes the real form, so its own `Load`/`Shown` code runs — a splash screen, docking panel or dialog opened there would otherwise appear next to VS Code. Rendering is unaffected. Turn off only to diagnose a vendor control; takes effect when the preview engine restarts. |
| `winformsDesigner.net48.releaseOnExternalBuild` | `true` | Hand the **net48** build output back automatically when a build started **outside** VS Code (Visual Studio, an external `msbuild`) is about to overwrite it, so that build never fails with `MSB3027`. Watches the pinned output (and the project's `obj`) and acts only when an assembly is actually written; the previews go view-only until the build lands, then re-render from it. |
...
**`net4x` build coordination.** The preview renders a *real compiled instance* of your form and therefore loads your assemblies in place (shadow-copying would break delay-signed vendor controls). A build started **outside** VS Code — Visual Studio, an external `msbuild` — is detected as it compiles and the output is handed back before the copy needs it, so it does not fail with `MSB3027` (`winformsDesigner.net48.releaseOnExternalBuild`, on by default). Use **WinForms: Run Build Task** / **Run Test Task** — `Ctrl+Shift+B` is routed through the coordinated build command while the designer is active — to release the output before the task, invalidate the compiled fallback, and re-render afterward. Build/test tasks launched elsewhere also trigger best-effort lifecycle coordination; **Release .NET Framework Assembly (for Rebuild)** remains available as a manual recovery control. The modern .NET engine loads your assemblies from an in-memory copy and never pins your managed build output; the one exception is a project-local **native** dll that a design-time constructor actually P/Invokes, which Windows maps in place — replacing that particular file still needs **Restart the Designer Preview Engine**.
...
safe source-first tab ordering in 1.7.0, exact placement, a self-maintaining workspace toolbox plus design-time
safety in 1.8.0 (opening a form no longer runs your application, holds your build output, or puts windows on your
screen), and the daily naming/navigation/default-event keyboard loop plus Explorer project-item creation in 1.9.0 —
plus the planned line: multi-object properties (1.10.0), precision layout (1.11.0), toolbox creation/placement
(1.12.0), assets and menu
productivity (1.13.0), visual-inheritance overrides (1.14.0), bounded data-binding productivity (1.15.0), and the extensible
design-time host planned for 2.0.0. The detailed [v2.0.0 implementation blueprint](docs/roadmap-v2.0.0-implementation-plan.md) defines the Visual Studio WinForms Designer parity matrix, architecture, execution waves, risks, verification corpus, and release-claim gates.

The safety core has fast C# and TypeScript unit coverage; the webview UI is validated headless (634 checks
across 159 tests), startup/render latency is guarded by a repeatable performance baseline, and activation,
